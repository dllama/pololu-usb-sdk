    ...
    internal static class Winusb
    {
        ...
        internal static unsafe WinUsbDeviceHandles listConnect(IntPtr listHandle, Byte index, Guid deviceInterfaceGuid)
        {
            ...
            if (handles.deviceHandle.IsInvalid)
            {
                ...
                {
                    throw new Win32Exception(error, "Access denied when trying to open device.  Try closing all other programs that are using the device.");
                }
                else
                {
        ...
        [DllImport("setupapi.dll", SetLastError = true, ThrowOnUnmappableChar = true)]
